...
//商品列表的位置转移就是把位置固定为style
window.onload = function(){
	var glist = document.getElementById('glist')
	var liAll = glist.getElementsByTagName('li')
	for(var i = 0; i<liAll.length;i++){
		liAll[i].style.left = liAll[i].offsetLeft + 'px';
		liAll[i].style.top = liAll[i].offsetTop + 'px'; 
	}
	for(var i = 0; i<liAll.length;i++){
		liAll[i].style.position = 'absolute';
		liAll[i].style.margin = '0px';
		liAll[i].style.zIndex = 1;
	}
